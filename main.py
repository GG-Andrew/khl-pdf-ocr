...
def normalize_cyrillic(text: str) -> str:
    ...
    # вставляем пробел перед инициалами/именем, если склеено: "БелоусовГеоргий" -> "Белоусов Георгий"
    s = re.sub(r"([А-ЯЁ][а-яё]+)([А-ЯЁ][а-яё]{2,})", r"\1 \2", s)
    ...
    # разгребаем пробелы/пустые строки
    s = re.sub(r"[ \t]+", " ", s)
    s = re.sub(r"\n{3,}", "\n\n", s)
    return s.strip()
